      ...
      <div class="space-y-4">
        ...
        <div class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
          <div class="flex items-start justify-between">
            ...
            <form action="<?= base_url('admin/verify/' . $ticket['job_ticket_response_id']) ?>" method="POST" class="ml-4">
              <?= csrf_field() ?>
              <button type="submit" onclick="return confirm('Verify and close this ticket?')"
                class="px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold text-xs rounded-xl shadow hover:shadow-lg transition-all whitespace-nowrap">
                Verify & Close
              </button>
            </form>
          </div>
