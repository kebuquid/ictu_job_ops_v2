...
<div class="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
      ...
      <div class="md:col-span-2 bg-blue-50/50 p-4 rounded-xl border border-blue-100">
        <span class="text-blue-700 text-xs font-bold uppercase tracking-wider block mb-2">Linked Asset Information</span>
  ...
  <!-- Response Form -->
  <form action="<?= base_url('admin/respond/' . $response['job_ticket_response_id']) ?>" method="POST" class="fade-in delay-2 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg space-y-5">
    <?= csrf_field() ?>
    ...
    <div>
      ...
      <textarea name="action_performed" rows="4" required
        class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all resize-none"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-3 pt-2">
      <button type="submit" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-bold text-sm rounded-xl shadow-lg hover:shadow-xl transition-all">
        Submit Response
      </button>
      <a href="<?= base_url('admin/my-tickets') ?>" class="px-6 py-3 bg-gray-100 text-gray-600 font-semibold text-sm rounded-xl hover:bg-gray-200 transition-all">
        Cancel
      </a>
    </div>
  </form>
