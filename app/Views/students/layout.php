...
<head>
  ...
  <title>ICTU Job Ticketing – Student</title>
  ...
  <style>
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .ticket-row:hover { background: #faf5ff; }
    .ticket-row { transition: background 0.15s; }
    .sidebar-active { background: linear-gradient(135deg, #9333ea, #7c3aed); color: white; box-shadow: 0 4px 15px rgba(147, 51, 234, 0.35); }
    @keyframes countUp { from { opacity: 0; transform: scale(0.5); } to { opacity: 1; transform: scale(1); } }
    .count-anim { animation: countUp 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
  </style>
</head>
<body class="bg-gradient-to-br from-purple-50 via-white to-violet-50 min-h-screen overflow-hidden">

  <div class="fixed top-0 right-0 w-96 h-96 bg-purple-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob pointer-events-none"></div>
  <div class="fixed bottom-0 left-0 w-96 h-96 bg-violet-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000 pointer-events-none"></div>
  <div class="fixed top-1/2 left-1/3 w-96 h-96 bg-fuchsia-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000 pointer-events-none"></div>
  ...
  <div class="flex h-screen overflow-hidden relative z-10">
    ...
    <aside class="w-72 bg-white/80 backdrop-blur-xl shadow-2xl flex flex-col border-r border-purple-100/50 shrink-0">
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        <p class="text-xs font-700 text-purple-400 uppercase tracking-widest px-3 pb-1 pt-2">Student Portal</p>

        <a href="<?= base_url('student/dashboard') ?>" class="sidebar-item <?= uri_string() === 'student/dashboard' ? 'sidebar-active' : 'text-gray-600 hover:text-purple-800 hover:bg-purple-50' ?> flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm">
        ...
        <a href="<?= base_url('student/create-ticket') ?>" class="sidebar-item <?= uri_string() === 'student/create-ticket' ? 'sidebar-active' : 'text-gray-600 hover:text-purple-800 hover:bg-purple-50' ?> flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition-all">
        ...
        <a href="<?= base_url('student/my-tickets') ?>" class="sidebar-item <?= str_starts_with(uri_string(), 'student/my-tickets') ? 'sidebar-active' : 'text-gray-600 hover:text-purple-800 hover:bg-purple-50' ?> flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition-all">
        ...
        <a href="#" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:text-purple-800 hover:bg-purple-50 font-medium text-sm transition-all">
      ...
      <div class="p-4 border-t border-purple-100/60">
    ...
    <main class="flex-1 overflow-y-auto">
      ...
      <div class="text-center pb-2">
        <p class="text-purple-300 text-xs">CSPC – ICTU Job Ticketing System &nbsp;•&nbsp; Version 2.0 – JobIgniter</p>
      </div>
    </main>
  </div>
