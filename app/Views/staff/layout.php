...
<head>
  ...
  <title>ICTU Job Ticketing - Staff</title>
  ...
  <style>
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .ticket-row:hover { background: #fffbeb; }
    .ticket-row { transition: background 0.15s; }
    .sidebar-active { background: linear-gradient(135deg, #d97706, #b45309); color: white; box-shadow: 0 4px 15px rgba(217, 119, 6, 0.35); }
    @keyframes countUp { from { opacity: 0; transform: scale(0.5); } to { opacity: 1; transform: scale(1); } }
    .count-anim { animation: countUp 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
  </style>
</head>
<body class="bg-gradient-to-br from-amber-50 via-white to-yellow-100 min-h-screen overflow-hidden">

  <div class="fixed top-0 right-0 w-96 h-96 bg-amber-300 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob pointer-events-none"></div>
  <div class="fixed bottom-0 left-0 w-96 h-96 bg-yellow-300 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000 pointer-events-none"></div>
  <div class="fixed top-1/2 left-1/3 w-96 h-96 bg-orange-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000 pointer-events-none"></div>

  <div class="flex h-screen overflow-hidden relative z-10">

    <aside class="w-72 bg-white/80 backdrop-blur-xl shadow-2xl flex flex-col border-r border-amber-100/50 shrink-0">
      <div class="p-6 border-b border-amber-100/60">
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        <a href="<?= base_url('staff/dashboard') ?>" class="sidebar-item <?= uri_string() === 'staff/dashboard' ? 'sidebar-active' : 'text-gray-600 hover:text-amber-700 hover:bg-amber-50' ?> flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm">
        ...
        <a href="<?= base_url('staff/my-tickets') ?>" class="sidebar-item <?= str_starts_with(uri_string(), 'staff/my-tickets') ? 'sidebar-active' : 'text-gray-600 hover:text-amber-700 hover:bg-amber-50' ?> flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition-all">
    ...
    <main class="flex-1 overflow-y-auto">
      <header class="bg-white/70 backdrop-blur-xl border-b border-amber-100/50 px-8 py-4 flex items-center gap-4 sticky top-0 z-20">
        <div class="flex-1">
        ...
        <button class="relative w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center hover:bg-amber-100 transition-colors">
  ...
  <div class="fixed bottom-0 left-0 w-full pointer-events-none opacity-20 z-0">
    <svg viewBox="0 0 1440 120" class="w-full h-auto">
      <path d="M0,64 C240,96 480,32 720,64 C960,96 1200,32 1440,64 L1440,120 L0,120 Z" fill="#f59e0b"/>
    </svg>
  </div>
</body>
</html>
