...
<div class="p-8 space-y-6 max-w-3xl mx-auto">
  ...
  <!-- Ticket Info Card -->
  <div class="fade-in bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-amber-100/50 shadow-lg">
    ...
    <div class="grid grid-cols-2 gap-4 text-sm">
      <div>
        ...
        <p class="font-bold text-amber-600 mono">ICTU-<?= date('Y', strtotime($response['created_at'])) ?>-<?= str_pad($response['job_ticket_id'], 5, '0', STR_PAD_LEFT) ?></p>
      </div>
    ...
    <form action="<?= base_url($urlPrefix . '/transfer/' . $response['job_ticket_response_id']) ?>" method="post">
      ...
      <div class="space-y-3">
        ...
        <?php else: ?>
          ...
          <label class="flex items-center gap-4 p-4 border border-gray-200 rounded-xl hover:border-amber-300 hover:bg-amber-50/50 cursor-pointer transition-all group">
            <input type="radio" name="new_staff_id" value="<?= $emp['user_id'] ?>" required class="text-amber-600 focus:ring-amber-500">
            <div class="flex-1">
              <p class="font-semibold text-gray-800 group-hover:text-amber-700"><?= esc($emp['name']) ?></p>
      ...
      <div class="flex items-center gap-3 mt-6">
        <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">
          Transfer Ticket
        </button>
