...
#expertiseTable tbody tr {
    background-color: #ffffff !important;
    color: #334155 !important;            /* slate-700 */
}

#expertiseTable tbody tr.odd {
    background-color: #ffffff !important;
}

#expertiseTable tbody tr.even {
...
<div class="p-8 space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    <div class="flex items-center justify-between mb-5">
      <h3 class="font-bold text-gray-900 text-lg">Expertise / Skills</h3>
      <a href="<?= base_url('super-admin/expertise/add') ?>"
         class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">+ Add Expertise</a>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm" id="expertiseTable">
        <thead>
        ...
        <tbody>
            <?php foreach ($expertiseList as $item): ?>
              <tr class="ticket-row border-b border-gray-50">
                <td class="py-3 pr-4">
                  <span class="text-gray-400 font-semibold text-xs"><?= esc($item['expertise_id']) ?></span>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-400 to-purple-600 text-white text-sm flex items-center justify-center font-bold shadow-sm shrink-0">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
                    </div>
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($item['skill']) ?></span>
                  </div>
                </td>
                <td class="py-3 pr-4">
                ...
                <td class="py-3 pr-4">
                  <span class="inline-block px-2.5 py-1 bg-<?= esc($item['section_color']) ?>-100 text-<?= esc($item['section_color']) ?>-600 text-xs font-bold rounded-full"><?= esc($item['acronym'] ?? '—') ?></span>
                </td>
                <td class="py-3">
                  <div class="flex items-center gap-2">
                    <a href="<?= base_url('super-admin/expertise/edit/' . $item['expertise_id']) ?>"
                    ...
                    <button type="button"
                            onclick="confirmDelete(<?= $item['expertise_id'] ?>, '<?= esc($item['skill'], 'js') ?>')"
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
  <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 transform transition-all">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h4 class="font-bold text-gray-900">Delete Expertise</h4>
        <p class="text-xs text-gray-500">This action cannot be undone.</p>
      </div>
    </div>
    <p class="text-sm text-gray-600 mb-5">Are you sure you want to delete <strong id="deleteExpertiseName" class="text-gray-900"></strong>?</p>
...
$(document).ready(function () {
  if($.fn.dataTable) {
    $('#expertiseTable').DataTable({
      ...
      order: [[0, 'desc']],
      language: {
        emptyTable: 'No expertise found. Add one to get started.'
      }
    });
  }
});
...
function confirmDelete(id, name) {
  $('#deleteExpertiseName').text(name);
  $('#deleteForm').attr('action', '<?= base_url("super-admin/expertise/delete") ?>/' + id);
