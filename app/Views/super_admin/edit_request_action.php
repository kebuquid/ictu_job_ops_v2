...
<div class="p-8 space-y-6 max-w-2xl mx-auto">
  ...
  <!-- Page Header -->
  <div class="fade-in">
    <a href="<?= base_url('super-admin/request-actions') ?>" class="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 font-medium mb-4 transition-colors">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      Back to Request Actions
    </a>
    <h2 class="text-2xl font-extrabold text-gray-900">Edit Request Action</h2>
    <p class="text-sm text-gray-500 mt-1">Update the request action details below.</p>
  </div>
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl border border-blue-100/50 shadow-lg p-6">
    ...
    <div class="flex items-center gap-4 mb-6 pb-5 border-b border-gray-100">
      <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-400 to-emerald-600 text-white flex items-center justify-center shadow-lg">
        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
      </div>
      <div>
        <p class="text-lg font-extrabold text-gray-900"><?= esc($requestAction['action_name']) ?></p>
        <p class="text-xs text-gray-400 mono">Action ID: <?= esc($requestAction['action_id']) ?></p>
      </div>
    </div>

    <form action="<?= base_url('super-admin/request-actions/edit/' . $requestAction['action_id']) ?>" method="POST" class="space-y-5">
      <?= csrf_field() ?>
      ...
      <div>
        ...
        <div class="relative">
          <div class="absolute left-3 top-1/2 -translate-y-1/2">
            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </div>
          <input
            type="text"
            name="action_name"
            id="actionName"
            required
            value="<?= old('action_name', $requestAction['action_name']) ?>"
        ...
        <div class="relative">
          ...
          >
            ...
            <?php foreach ($requestTypes as $requestType): ?>
              <option value="<?= esc($requestType['request_type_id']) ?>" <?= old('request_type_id', $requestAction['request_type_id']) == $requestType['request_type_id'] ? 'selected' : '' ?>>
        ...
        <div class="relative">
          ...
          >
            ...
            <?php foreach ($sections as $section): ?>
              <option value="<?= esc($section['section_id']) ?>" <?= old('section_id', $requestAction['section_id']) == $section['section_id'] ? 'selected' : '' ?>>
      ...
      <div class="flex gap-3 pt-2">
        <a href="<?= base_url('super-admin/request-actions') ?>"
