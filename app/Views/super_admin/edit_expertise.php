...
<div class="p-8 space-y-6 max-w-2xl mx-auto">
  ...
  <div class="fade-in">
    ...
    <h2 class="text-2xl font-extrabold text-gray-900">Edit Expertise</h2>
    <p class="text-sm text-gray-500 mt-1">Update the expertise details below.</p>
  </div>
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl border border-blue-100/50 shadow-lg p-6">
    ...
    <div class="flex items-center gap-4 mb-6 pb-5 border-b border-gray-100">
      <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-400 to-purple-600 text-white flex items-center justify-center shadow-lg">
        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
      </div>
      <div>
        <p class="text-lg font-extrabold text-gray-900"><?= esc($expertise['skill']) ?></p>
        <p class="text-xs text-gray-400 mono">Expertise ID: <?= esc($expertise['expertise_id']) ?></p>
      </div>
    </div>

    <form action="<?= base_url('super-admin/expertise/edit/' . $expertise['expertise_id']) ?>" method="POST" class="space-y-5">
      <?= csrf_field() ?>
      ...
      <div>
        ...
        <div class="relative">
          ...
          <input
            ...
            id="skillName"
            required
            value="<?= old('skill', $expertise['skill']) ?>"
        ...
        <div class="relative">
          ...
          >
            ...
            <?php foreach ($sections as $section): ?>
              <option value="<?= esc($section['section_id']) ?>" <?= old('section_id', $expertise['section_id']) == $section['section_id'] ? 'selected' : '' ?>>
      ...
      <div>
        ...
        ><?= old('description', $expertise['description']) ?></textarea>
      </div>

      <!-- Buttons -->
      <div class="flex gap-3 pt-2">
        <a href="<?= base_url('super-admin/expertise') ?>"
