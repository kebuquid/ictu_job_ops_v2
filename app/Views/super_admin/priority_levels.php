...
#priorityLevelsTable tbody tr {
    background-color: #ffffff !important;
    color: #334155 !important;            /* slate-700 */
}

#priorityLevelsTable tbody tr.odd {
    background-color: #ffffff !important;
}

#priorityLevelsTable tbody tr.even {
...
<div class="p-8 space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    <div class="flex items-center justify-between mb-5">
      <h3 class="font-bold text-gray-900 text-lg">Priority Levels</h3>
      <a href="<?= base_url('super-admin/priority-levels/add') ?>"
         class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">+ Add Priority Level</a>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm" id="priorityLevelsTable">
        <thead>
          <tr class="text-left text-xs text-gray-400 uppercase tracking-wider border-b border-gray-100">
            <th class="pb-3 pr-4">#</th>
            <th class="pb-3 pr-4">Priority Name</th>
            <th class="pb-3 pr-4">Operation Status</th>
            <th class="pb-3 pr-4">Description</th>
            <th class="pb-3">Actions</th>
          </tr>
        </thead>
        <tbody>
            <?php foreach ($priorityLevels as $item): ?>
              <tr class="ticket-row border-b border-gray-50">
                <td class="py-3 pr-4">
                  <span class="text-gray-400 font-semibold text-xs"><?= esc($item['priority_level_id']) ?></span>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-red-400 to-rose-600 text-white text-sm flex items-center justify-center font-bold shadow-sm shrink-0">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    </div>
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($item['priority_name']) ?></span>
                  </div>
                </td>
                <td class="py-3 pr-4">
                  <span class="text-gray-600 text-sm"><?= esc($item['operation_status'] ?? '—') ?></span>
                </td>
                <td class="py-3 pr-4">
                ...
                <td class="py-3">
                  <div class="flex items-center gap-2">
                    <a href="<?= base_url('super-admin/priority-levels/edit/' . $item['priority_level_id']) ?>"
                    ...
                    <button type="button"
                            onclick="confirmDelete(<?= $item['priority_level_id'] ?>, '<?= esc($item['priority_name'], 'js') ?>')"
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
  <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 transform transition-all">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h4 class="font-bold text-gray-900">Delete Priority Level</h4>
        <p class="text-xs text-gray-500">This action cannot be undone.</p>
      </div>
    </div>
    <p class="text-sm text-gray-600 mb-5">Are you sure you want to delete <strong id="deletePriorityLevelName" class="text-gray-900"></strong>?</p>
...
$(document).ready(function () {
  if($.fn.dataTable) {
    $('#priorityLevelsTable').DataTable({
      pageLength: 15,
      order: [[0, 'desc']],
      language: {
        emptyTable: 'No priority levels found. Add one to get started.'
      }
    });
  }
});
...
function confirmDelete(id, name) {
  $('#deletePriorityLevelName').text(name);
  $('#deleteForm').attr('action', '<?= base_url("super-admin/priority-levels/delete") ?>/' + id);
