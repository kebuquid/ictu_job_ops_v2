...
#issueTypesTable tbody tr {
    background-color: #ffffff !important;
    color: #334155 !important;            /* slate-700 */
}

#issueTypesTable tbody tr.odd {
    background-color: #ffffff !important;
}

#issueTypesTable tbody tr.even {
...
<div class="p-8 space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    <div class="flex items-center justify-between mb-5">
      <h3 class="font-bold text-gray-900 text-lg">Issue Types</h3>
      <a href="<?= base_url('super-admin/issue-types/add') ?>"
         class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">+ Add Issue Type</a>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm" id="issueTypesTable">
        <thead>
        ...
        <tbody>
            <?php foreach ($issueTypes as $item): ?>
              <tr class="ticket-row border-b border-gray-50">
                <td class="py-3 pr-4">
                  <span class="text-gray-400 font-semibold text-xs"><?= esc($item['issue_type_id']) ?></span>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-rose-400 to-pink-600 text-white text-sm flex items-center justify-center font-bold shadow-sm shrink-0">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($item['issue_type_name']) ?></span>
                  </div>
                </td>
                <td class="py-3 pr-4">
                  <span class="text-gray-600 text-sm"><?= esc($item['issue_type_domain'] ?? '—') ?></span>
                </td>
                <td class="py-3 pr-4">
                ...
                <td class="py-3">
                  <div class="flex items-center gap-2">
                    <a href="<?= base_url('super-admin/issue-types/edit/' . $item['issue_type_id']) ?>"
                    ...
                    <button type="button"
                            onclick="confirmDelete(<?= $item['issue_type_id'] ?>, '<?= esc($item['issue_type_name'], 'js') ?>')"
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
  <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 transform transition-all">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h4 class="font-bold text-gray-900">Delete Issue Type</h4>
        <p class="text-xs text-gray-500">This action cannot be undone.</p>
      </div>
    </div>
    <p class="text-sm text-gray-600 mb-5">Are you sure you want to delete <strong id="deleteIssueTypeName" class="text-gray-900"></strong>?</p>
...
$(document).ready(function () {
  if($.fn.dataTable) {
    $('#issueTypesTable').DataTable({
      pageLength: 15,
      order: [[0, 'desc']],
      language: {
        emptyTable: 'No issue types found. Add one to get started.'
      }
    });
  }
});
...
function confirmDelete(id, name) {
  $('#deleteIssueTypeName').text(name);
  $('#deleteForm').attr('action', '<?= base_url("super-admin/issue-types/delete") ?>/' + id);
