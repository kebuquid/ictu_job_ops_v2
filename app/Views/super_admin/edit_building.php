...
<div class="p-8 space-y-6 max-w-2xl mx-auto">
  ...
  <!-- Page Header -->
  <div class="fade-in">
    <a href="<?= base_url('super-admin/buildings') ?>" class="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 font-medium mb-4 transition-colors">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      Back to Buildings
    </a>
    <h2 class="text-2xl font-extrabold text-gray-900">Edit Building</h2>
    <p class="text-sm text-gray-500 mt-1">Update the building details below.</p>
  </div>
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl border border-blue-100/50 shadow-lg p-6">
    ...
    <div class="flex items-center gap-4 mb-6 pb-5 border-b border-gray-100">
      <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 text-white flex items-center justify-center shadow-lg">
        <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
      </div>
      <div>
        <p class="text-lg font-extrabold text-gray-900"><?= esc($building['name']) ?></p>
        <p class="text-xs text-gray-400 mono">Building ID: <?= esc($building['building_id']) ?></p>
      </div>
    </div>

    <form action="<?= base_url('super-admin/buildings/edit/' . $building['building_id']) ?>" method="POST" class="space-y-5">
      <?= csrf_field() ?>
      <input type="hidden" name="_method" value="PUT">

      <!-- Building Name -->
      <div>
        <label for="buildingName" class="block text-sm font-semibold text-gray-700 mb-2">Building Name</label>
        <div class="relative">
          ...
          <input
            ...
            placeholder="e.g. Administration Building, Science Hall..."
      ...
      <div class="flex gap-3 pt-2">
        <a href="<?= base_url('super-admin/buildings') ?>"
