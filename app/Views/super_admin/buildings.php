...
#buildingsTable tbody tr.odd {
    background-color: #ffffff !important;
}

#buildingsTable tbody tr.even {
...
<div class="p-8 space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    <div class="flex items-center justify-between mb-5">
      <h3 class="font-bold text-gray-900 text-lg">Buildings</h3>
      <a href="<?= base_url('super-admin/buildings/add') ?>"
         class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">+ Add Building</a>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm" id="buildingsTable">
        <thead>
          <tr class="text-left text-xs text-gray-400 uppercase tracking-wider border-b border-gray-100">
            <th class="pb-3 pr-4">#</th>
            <th class="pb-3 pr-4">Building Name</th>
        ...
        <tbody>
            <?php foreach ($buildings as $building): ?>
              <tr class="ticket-row border-b border-gray-50">
                <td class="py-3 pr-4">
                  <span class="text-gray-400 font-semibold text-xs"><?= esc($building['building_id']) ?></span>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 text-white text-sm flex items-center justify-center font-bold shadow-sm shrink-0">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    </div>
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($building['name']) ?></span>
                  </div>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($building['created_at']) ?></span>
                  </div>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($building['updated_at']) ?></span>
                  </div>
                </td>
                <td class="py-3">
                  <div class="flex items-center gap-2">
                    <a href="<?= base_url('super-admin/buildings/edit/' . $building['building_id']) ?>"
                    ...
                    <button type="button"
                            onclick="confirmDelete(<?= $building['building_id'] ?>, '<?= esc($building['name'], 'js') ?>')"
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
  <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 transform transition-all">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h4 class="font-bold text-gray-900">Delete Building</h4>
        <p class="text-xs text-gray-500">This action cannot be undone.</p>
      </div>
    </div>
    <p class="text-sm text-gray-600 mb-5">Are you sure you want to delete <strong id="deleteBuildingName" class="text-gray-900"></strong>?</p>
...
$(document).ready(function () {
  if($.fn.dataTable) {
    $('#buildingsTable').DataTable({
      ...
      order: [[0, 'desc']],
      language: {
        emptyTable: 'No buildings found. Add one to get started.'
      }
    });
  }
});
...
function confirmDelete(id, name) {
  ...
  $('#deleteForm').attr('action', '<?= base_url("super-admin/buildings/delete") ?>/' + id);
