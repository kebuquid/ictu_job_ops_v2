...
#orgUnitsTable tbody tr {
    background-color: #ffffff !important;
    color: #334155 !important;            /* slate-700 */
}

#orgUnitsTable tbody tr.odd {
    background-color: #ffffff !important;
}

#orgUnitsTable tbody tr.even {
...
<div class="p-8 space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    <div class="flex items-center justify-between mb-5">
      <h3 class="font-bold text-gray-900 text-lg">Organizational Units</h3>
      <a href="<?= base_url('super-admin/organizational-units/add') ?>"
         class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">+ Add Organizational Unit</a>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm" id="orgUnitsTable">
        <thead>
          <tr class="text-left text-xs text-gray-400 uppercase tracking-wider border-b border-gray-100">
            <th class="pb-3 pr-4">#</th>
            <th class="pb-3 pr-4">Unit Name</th>
            <th class="pb-3 pr-4">Description</th>
            <th class="pb-3 pr-4">Building</th>
            <th class="pb-3">Actions</th>
          </tr>
        </thead>
        <tbody>
            <?php foreach ($orgUnits as $item): ?>
              <tr class="ticket-row border-b border-gray-50">
                <td class="py-3 pr-4">
                  <span class="text-gray-400 font-semibold text-xs"><?= esc($item['unit_id']) ?></span>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-400 to-cyan-600 text-white text-sm flex items-center justify-center font-bold shadow-sm shrink-0">
                ...
                <td class="py-3 pr-4">
                  <span class="inline-block px-2.5 py-1 bg-amber-100 text-amber-700 text-xs font-bold rounded-full"><?= esc($item['building_name'] ?? '—') ?></span>
                </td>
                <td class="py-3">
                  <div class="flex items-center gap-2">
                    <a href="<?= base_url('super-admin/organizational-units/edit/' . $item['unit_id']) ?>"
                    ...
                    <button type="button"
                            onclick="confirmDelete(<?= $item['unit_id'] ?>, '<?= esc($item['name'], 'js') ?>')"
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
  <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 transform transition-all">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h4 class="font-bold text-gray-900">Delete Organizational Unit</h4>
        <p class="text-xs text-gray-500">This action cannot be undone.</p>
      </div>
    </div>
    <p class="text-sm text-gray-600 mb-5">Are you sure you want to delete <strong id="deleteOrgUnitName" class="text-gray-900"></strong>?</p>
...
$(document).ready(function () {
  if($.fn.dataTable) {
    $('#orgUnitsTable').DataTable({
      pageLength: 15,
      order: [[0, 'desc']],
      language: {
        emptyTable: 'No organizational units found. Add one to get started.'
      }
    });
  }
});
...
function confirmDelete(id, name) {
  $('#deleteOrgUnitName').text(name);
  $('#deleteForm').attr('action', '<?= base_url("super-admin/organizational-units/delete") ?>/' + id);
