...
<div class="p-8 space-y-6 max-w-2xl mx-auto">
  ...
  <!-- Page Header -->
  <div class="fade-in">
    <a href="<?= base_url('super-admin/ticket-equipment') ?>" class="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 font-medium mb-4 transition-colors">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      Back to Ticket Equipment
    </a>
    <h2 class="text-2xl font-extrabold text-gray-900">Edit Equipment</h2>
    <p class="text-sm text-gray-500 mt-1">Update the equipment details below.</p>
  </div>
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl border border-blue-100/50 shadow-lg p-6">
    ...
    <div class="flex items-center gap-4 mb-6 pb-5 border-b border-gray-100">
      <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-yellow-400 to-amber-600 text-white flex items-center justify-center shadow-lg">
      ...
      <div>
        <p class="text-lg font-extrabold text-gray-900"><?= esc($equipment['name']) ?></p>
        <p class="text-xs text-gray-400 mono">Equipment ID: <?= esc($equipment['equipment_id']) ?></p>
      </div>
    </div>

    <form action="<?= base_url('super-admin/ticket-equipment/edit/' . $equipment['equipment_id']) ?>" method="POST" class="space-y-5">
      <?= csrf_field() ?>
      <input type="hidden" name="_method" value="PUT">

      <!-- Equipment Name -->
      <div>
        <label for="equipmentName" class="block text-sm font-semibold text-gray-700 mb-2">Equipment Name</label>
        <div class="relative">
          ...
          <input
            ...
            placeholder="e.g. Printer, Router, Monitor..."
        ...
        <div class="relative">
          ...
          >
            ...
            <?php foreach ($sections as $section): ?>
              <option value="<?= esc($section['section_id']) ?>" <?= old('section_id', $equipment['section_id']) == $section['section_id'] ? 'selected' : '' ?>>
      ...
      <div class="flex gap-3 pt-2">
        <a href="<?= base_url('super-admin/ticket-equipment') ?>"
