...
#requestPlatformsTable, 
#requestPlatformsTable thead th {
...
#requestPlatformsTable tbody tr {
    background-color: #ffffff !important;
    color: #334155 !important;            /* slate-700 */
}

#requestPlatformsTable tbody tr.odd {
    background-color: #ffffff !important;
}

#requestPlatformsTable tbody tr.even {
...
<div class="p-8 space-y-6">
  ...
  <div class="fade-in delay-1 bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-blue-100/50 shadow-lg">
    <div class="flex items-center justify-between mb-5">
      <h3 class="font-bold text-gray-900 text-lg">Request Platforms</h3>
      <a href="<?= base_url('super-admin/request-platforms/add') ?>"
         class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-semibold text-sm rounded-xl shadow hover:shadow-lg transition-all">+ Add Request Platform</a>
    </div>
    <div class="overflow-x-auto">
      <table class="w-full text-sm" id="requestPlatformsTable">
        <thead>
          <tr class="text-left text-xs text-gray-400 uppercase tracking-wider border-b border-gray-100">
            <th class="pb-3 pr-4">#</th>
            <th class="pb-3 pr-4">Platform Name</th>
        ...
        <tbody>
            <?php foreach ($requestPlatforms as $item): ?>
              <tr class="ticket-row border-b border-gray-50">
                <td class="py-3 pr-4">
                  <span class="text-gray-400 font-semibold text-xs"><?= esc($item['platform_id']) ?></span>
                </td>
                <td class="py-3 pr-4">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-sky-400 to-blue-600 text-white text-sm flex items-center justify-center font-bold shadow-sm shrink-0">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    </div>
                    <span class="text-gray-800 font-semibold text-sm"><?= esc($item['platform_name']) ?></span>
                  </div>
                </td>
                <td class="py-3 pr-4">
                  <p class="text-gray-600 text-xs max-w-xs truncate"><?= esc($item['platform_description'] ?? '—') ?></p>
                </td>
                <td class="py-3 pr-4">
                ...
                <td class="py-3">
                  <div class="flex items-center gap-2">
                    <a href="<?= base_url('super-admin/request-platforms/edit/' . $item['platform_id']) ?>"
                    ...
                    <button type="button"
                            onclick="confirmDelete(<?= $item['platform_id'] ?>, '<?= esc($item['platform_name'], 'js') ?>')"
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
  <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 transform transition-all">
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h4 class="font-bold text-gray-900">Delete Request Platform</h4>
        <p class="text-xs text-gray-500">This action cannot be undone.</p>
      </div>
    </div>
    <p class="text-sm text-gray-600 mb-5">Are you sure you want to delete <strong id="deleteRequestPlatformName" class="text-gray-900"></strong>?</p>
...
$(document).ready(function () {
  if($.fn.dataTable) {
    $('#requestPlatformsTable').DataTable({
      pageLength: 15,
      order: [[0, 'desc']],
      language: {
        emptyTable: 'No request platforms found. Add one to get started.'
      }
    });
  }
});
...
function confirmDelete(id, name) {
  $('#deleteRequestPlatformName').text(name);
  $('#deleteForm').attr('action', '<?= base_url("super-admin/request-platforms/delete") ?>/' + id);
