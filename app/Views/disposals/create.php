...
<div class="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl px-5 py-3 text-sm flex gap-2 items-start mb-6">
    <i class="fa-solid fa-triangle-exclamation mt-0.5 flex-shrink-0"></i>
    <p>Recording a disposal will automatically update the asset's status to <strong>Disposed</strong>.</p>
</div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="asset_id" required
                    ...
                    <?php foreach ($assets as $a): ?>
                        <option value="<?= $a['asset_id'] ?>" <?= set_select('asset_id', (string)$a['asset_id']) ?>>
            ...
            <div>
                ...
                <input type="date" name="disposal_date"
                       value="<?= set_value('disposal_date', date('Y-m-d')) ?>"
                       required
                       class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">
            </div>
            <div>
                ...
                <select name="condition_status"
                    ...
                    <?php foreach (['Good', 'Fair', 'Poor', 'Beyond Repair', 'Lost', 'Stolen'] as $c): ?>
                        <option value="<?= $c ?>" <?= set_select('condition_status', $c) ?>><?= $c ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Approved By</label>
                <input type="hidden" name="approved_by" id="disp_approved_by_id" value="<?= set_value('approved_by') ?>">
                <div class="relative">
        ...
        <div class="mt-4">
            ...
            <textarea name="disposal_reason" required rows="4"
                      ...
                      placeholder="Why is this asset being disposed…"><?= set_value('disposal_reason') ?></textarea>
            <div id="kw-tip" class="hidden"></div>
        </div>        <!-- Disposal Image -->
        <div class="mt-4">
            <label class="block text-xs font-medium text-gray-600 mb-1">Disposal Image <span class="text-gray-400 font-normal">(optional)</span></label>
            <div id="image-drop-zone"
                ...
                <div id="image-placeholder">
                    ...
                    <p class="text-sm text-gray-400">Click or drag &amp; drop an image</p>
...
function validateDisposal() {
    ...
    document.querySelectorAll('#disposal-form .field-error').forEach(e => e.remove());
    document.querySelectorAll('#disposal-form .input-error').forEach(el => {
        el.classList.remove('border-red-400', 'ring-2', 'ring-red-200', 'input-error');
    });

    const required = [
        { sel: '[name="asset_id"]',         label: 'Asset' },
        { sel: '[name="disposal_date"]',    label: 'Disposal Date' },
        { sel: '[name="condition_status"]', label: 'Condition at Disposal' },
        { sel: '[name="disposal_reason"]',  label: 'Disposal Reason' },
    ];
