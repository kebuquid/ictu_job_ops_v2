...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-gray-100">
        ...
        <div class="flex items-center gap-2">
            <form method="get" action="<?= site_url('disposals') ?>" class="flex items-center gap-1">
                <div class="relative">
                    <span class="absolute inset-y-0 left-3 flex items-center text-gray-400 pointer-events-none">
                        <i class="fa-solid fa-magnifying-glass text-xs"></i>
                    </span>
                    <input type="text" name="q"
                           class="pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400 w-52"
                           placeholder="Search…"
                           value="<?= esc($keyword ?? '') ?>">
                </div>
                <?php if (!empty($keyword)): ?>
                    <a href="<?= site_url('disposals') ?>" class="p-2 text-gray-400 hover:text-red-500 transition">
                        <i class="fa-solid fa-xmark"></i>
                    </a>
                <?php endif; ?>
            </form>
            <a href="<?= site_url('disposals/create') ?>"
            ...
            <?php if (empty($records)): ?>
                <tr>
                    <td colspan="7" class="px-4 py-12 text-center text-gray-400">
                        ...
                        <a href="<?= site_url('disposals/create') ?>" class="text-red-500 hover:underline ml-1">Add one now</a>
                    </td>
                </tr>
            <?php else: ?>
